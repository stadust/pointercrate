...
class NationStatsViewer extends StatsViewer {
    ...
    onReceive(response) {
        ...
        formatInto(this._unbeaten, nationData.unbeaten.map(demon => this.formatDemon(demon, "/list/permalink/" + demon.id + "/")))
        ...
        formatInto(this._created, nationData.created.map(creation => {
            return this.makeTooltip(this.formatDemon({name: creation.demon, position: creation.position}, "/list/permalink/" + creation.id + "/"), "(Co)created&nbsp;by&nbsp;" + creation.players.length + "&nbsp;player" + (creation.players.length === 1 ? "" : "s") + "&nbsp;in&nbsp;this&nbsp;country: ", creation.players.join(", "));
        }));
        formatInto(this._verified, nationData.verified.map(verification => {
            return this.makeTooltip(this.formatDemon({name: verification.demon, position: verification.position}, "/list/permalink/" + verification.id + "/"), "Verified&nbsp;by: ", verification.player);
        }));
        formatInto(this._published, nationData.published.map(publication => {
            return this.makeTooltip(this.formatDemon({name: publication.demon, position: publication.position}, "/list/permalink/" + publication.id + "/"), "Published&nbsp;by: ", publication.player);
        }));
    }
    ...
    formatDemonFromRecord(record) {
        let baseElement = this.formatDemon({name: record.demon, position: record.position}, "/list/permalink/" + record.id + "/");
