...
class IndividualStatsViewer extends StatsViewer {
    ...
    formatDemonsInto(element, demons) {
        formatInto(element, demons.map(demon => this.formatDemon(demon, "/list/permalink/" + demon.id + "/")));
    }

    formatRecordsInto(element, records) {
        formatInto(element, records.map(record => {
            let demon = this.formatDemon(record.demon, record.video ?? ("/list/permalink/" + record.demon.id + "/"));
