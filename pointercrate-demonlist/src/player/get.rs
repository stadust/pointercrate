...
impl Player {
    ...
    pub async fn by_id(id: i32, connection: &mut PgConnection) -> Result<Player> {
        let result = sqlx::query!(
            r#"SELECT players.id, players.name, banned, players.score, players.unrated_score, nationalities.nation::text, iso_country_code::text, iso_code::text as subdivision_code, subdivisions.name::text as subdivision_name, player_ranks.rank, player_ranks.unrated_rank FROM players LEFT OUTER JOIN nationalities ON 
        ...
        match result {
            Ok(row) => {
                ...
                Ok(Player {
                    ...
                    score: row.unrated_score,
                    rated_rank: row.rank,
                    rank: row.unrated_rank,
                    nationality,
                })
            },
