...
impl Nationality {
    ...
    pub async fn update_nation_score(&self, connection: &mut PgConnection) -> Result<(), sqlx::Error> {
        sqlx::query!(
            "UPDATE nationalities SET score = coalesce(score_of_nation(true, $1), 0), unrated_score = coalesce(score_of_nation(false, $1), 0) WHERE iso_country_code = $1",
        ...
        if let Some(ref subdivision) = self.subdivision {
            sqlx::query!(
                "UPDATE subdivisions SET score = coalesce(score_of_subdivision(true, $1, $2), 0), unrated_score = coalesce(score_of_subdivision(false, $1, $2), 0) WHERE nation = $1 AND iso_code = $2",
                self.iso_country_code,
