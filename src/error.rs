...
#[derive(Debug)]
pub enum PointercrateErrorResponse {
    Default(PointercrateError),
    Html(maud::PreEscaped<String>),
}

impl std::fmt::Display for PointercrateErrorResponse {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "fuck off")
    }
}

impl std::error::Error for PointercrateErrorResponse {}

impl ResponseError for PointercrateErrorResponse {
    fn error_response(&self) -> HttpResponse {
        match self {
            PointercrateErrorResponse::Default(error) => {
                let mut response = HttpResponse::build(error.status_code());

                if let PointercrateError::MethodNotAllowed { allowed_methods } = error {
                    response.header("Allow", allowed_methods.join_with(",").to_string());
                }
            ...
            PointercrateErrorResponse::Html(html) =>
                HttpResponse::Ok()
